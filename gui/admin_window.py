...
class AdminWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Реестр администраторов")
        self.resize(300, 400)
        ...
        layout.addWidget(QLabel("Системные администраторы:"))
    ...
    def add_admin(self):
        ...
        try:
            cur = self._conn.cursor()
            cur.execute("SELECT COUNT(*) FROM admins WHERE LOWER(name) = LOWER(?)", (name.lower(),))
            if cur.fetchone()[0] > 0:
                QMessageBox.warning(self, "Ошибка", f"Адрес '{name}' уже есть в базе данных.")
                return

            cur.execute("INSERT INTO admins (name) VALUES (?)", (name,))
            self._conn.commit()
    ...
    def remove_selected(self):
        ...
        for item in items_to_remove:
            cur.execute("DELETE FROM admins WHERE name=?", (item.text(),))
        self._conn.commit()
        self.refresh()
